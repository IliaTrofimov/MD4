...
namespace MD4_hash
{
    ...
    public class MD4
    {
        ...
        /// <summary>
        /// Last value that was hashed. Changing it will reset last hash value.
        /// </summary>
        public string? Value
        {
            get => value;
        ...
        public byte[]? HashBytes => lastHash;

        /// <summary>
        /// Salt value. Changing it will reset last hash value.
        /// </summary>
        public string Salt
        {
            get => salt;
        ...
        public virtual byte[]? GetByteHash()
        {
            if (lastHash == null)
                GetByteHash(Value);
            return lastHash;
        ...
        /// <summary>
        ///   Returns a string that contains the hexadecimal hash oflast computed value
        /// </summary>
        /// <returns>String that contains the hex of the hash</returns>
        public virtual string GetHexHash(bool upperCase = false)
        {
            if (lastHash == null)
                GetByteHash();
        ...
        {
            if (h1.lastHash == null && h2.lastHash == null) 
                return true;
            else if (h1.lastHash != null && h2.lastHash != null)
            {
                for (int i = 0; i < h1.lastHash.Length && i < h2.lastHash.Length; i++)
                    if (h1.lastHash[i] != h2.lastHash[i]) return false;
                return true;
            }
